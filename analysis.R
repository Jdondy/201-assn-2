...
state_highest_ratio <- states[which.max(states$ratio),2]
...
min_cases <- filter(states, min(cases) == cases)
state_lowest_cases <- pull(min_cases, state)
...
county_highest_cases <- counties[which.max(counties$cases),2]
...
location_most_deaths <- counties[which.max(counties$deaths),7]
...
date_most_cases <- national[which.max(national$new_cases),1]

# What was the date when the most new deaths occured?
# `date_most_deaths`

date_most_deaths <- national[which.max(national$new_deaths),1]

# How many people died on the date when the most deaths occured? `most_deaths`

most_deaths <- max(national$new_deaths, na.rm = TRUE)
...
state_by_day <- states %>%
  group_by(date) %>%
  summarize(date, state_total = sum(cases))
...
county_by_day <- counties %>%
  group_by(date) %>%
  summarize(date, county_total = sum(cases))
...
sum_county_to_state <- counties %>%
  group_by(county,state,date, .add = TRUE) %>% 
  summarise(county_totals = sum(cases), date,.groups = "drop")
...
has_discrepancy <-  joined_states%>% 
  mutate(diff = abs(cases - county_totals)) %>% 
  filter(diff != 0) %>% 
  select(date,state, cases, county_totals, diff)
...
#Current highest number of deaths, nationally:
many_deaths <- max(national$deaths)

#current highest number of cases, nationally:
biggest_case <- max(national$cases)

#When was the first case discovered?:
first_case <- min(national$date)
